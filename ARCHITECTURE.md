...
**A restrained accent color instead of the default theme.** The frontend shipped on shadcn's unmodified grayscale palette for most of this project — functional, but indistinguishable from any other prototype. I first picked a blue accent for interactive elements specifically to avoid colliding with the green/red/amber gain/loss/pending states — then switched the accent itself to a deliberate trading-platform green (`oklch(0.5 0.19 152)` light, `oklch(0.58 0.18 152)` dark), the aesthetic this system is actually going for. It now shares a hue family with `--gain` on purpose — buttons, the active sidebar item, and rings read as the same "this app is green" brand, while staying a visibly different shade (lower lightness and chroma) from the brighter, purely semantic green `daily_trade`/positions actually gain or lose money in. Both a light and a dark palette are defined this way, switchable from the sidebar, not just a dark-mode-only app with the toggle removed.
